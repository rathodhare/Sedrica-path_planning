...
const int base_lidar_dist = 50;   //distance from vehicle centre(wheel axis) to lidar in cm(only in y-direction)
...
void laneCallback(nav_msgs::OccupancyGrid msg)
{
   ...
   for(i=0;i<l_height/2;i++)//initialising map
   {
      for(j=0;j<l_width;j++)
      {
   ...
   for(i=l_height-1;i>l_height/2;i--)//initialising map
   {
      for(j=0;j<l_width;j++)
      {
         int i1;
