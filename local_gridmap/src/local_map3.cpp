...
int l_width_new = 0,l_height_new = 0;

float l_res = 0.05;          

const int width =80/map_res;//map size
const int height = 60/map_res;
const int range = 50/map_res;//10m in cells
...
/*float min_r=12,min_ang=0;
   ...
   {
      int x_coord = round(cartesian[i].first/map_res)+height/2;
...
{
   ros::init(argc,argv,"local_map_node_3");
    ...
    int i,j,k,l;
       for(int i=0;i<l_height_new;i++)//adding lane data 
      {
         for(int j=0;j<l_width_new;j++)
         {
             ...
             grid_map.data[(i)*width+int(j+width/2-l_width_new/2)] = 100;
