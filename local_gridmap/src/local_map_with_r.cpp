...
const int radius_in_cm = 40;     //radius around on object to be in safe limit for path-planning
const int lane_in_cm = 5;       //radius around lane for path planning
...
const int range = 15/map_res;//10m in cells
   ...
   for(i=l_height-1;i>=0;i--)//initialising map
   {
      for(j=0;j<l_width;j++)
      {
         ...
         if(lane_map.data[i1*l_width+j]==2){ 
            ...
            int y_coord_lane = round(lane_cartesian[0].second/map_res) ; 
...
float min_r=12,min_ang=0;
   ...
   float major_axis_in_cells = minor_axis_in_cells+4;
   ...
   for(i=0;i<num_values;i++)
   {
      ...
      if(x_coord > 3 && x_coord <= range)//limiting obstacle detection to only 10m
      {
         if(y_coord <= range && y_coord >= -range)
         {
            ...
            //grid_map.data[x_coord*width+y_coord+width/2] = 0;
         } 
      }
   }
     ...
     for(int i=0;i<l_height_new;i++)//adding lane data 
      {
         for(int j=0;j<l_width_new;j++)
         {
             ...
             grid_map.data[(i)*width+int(j+width/2-l_width_new/2)] = 90;
   ...
   for(i=0;i<height;i++)//creating blown map
   {
      for(j=0;j<width;j++)
      {
         if(grid_map.data[i*width+j]==90)//blowing lane data
         {
               ...
               for(l=-1*minor_axis_in_cells;l<=minor_axis_in_cells;l++)
               {
                  if(((minor_axis_in_cells*minor_axis_in_cells)*(k*k) + (major_axis_in_cells*major_axis_in_cells)*(l*l)) < (minor_axis_in_cells*minor_axis_in_cells)*(major_axis_in_cells*major_axis_in_cells))//inside ellipse condition
                  {
                     ...
                     if(x >= 0 && x <= range)
                     {
                        if(y <= range && y >= -range)
                        {
                           blown_grid_map.data[x*width+y+width/2] = 90;
                        }
                     }
                  }
               }
