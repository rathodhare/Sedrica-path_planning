      ...
      for(j=0;j<l_width;j++)
      {
         ...
         if(lane_map.data[i1*l_width+j]==2){ 
           // x = ((l_height_new-((l_height-i1)*l_res/map_res))*l_width_new)+(((j- l_width/2)*l_res/map_res)+ l_width_new/2);
            x = ((int(i1*l_res/map_res))*l_width_new)  +  int((j)*l_res/map_res) ;
            //cout<<x<<endl;
...
void lidarCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
   ...
   for(i=0;i<num_values;i++)
   {
      ...
      if(x_coord > 0.5 && x_coord <= range)//limiting obstacle detection to only 10m
      {
     ...
     for(int i=0;i<l_height_new;i++)//adding lane data 
      {
         ...
         {
             if(lane_map_new.data[(i+base_cam_dist_in_cells)*l_width_new+j]==2)
               //if(lane_map_new.data[(i+base_cam_dist_in_cells)*l_width_new+j-1]!=2)
             grid_map.data[int(i*width+j+width/2-l_width_new/2-1)] = 70;
   ...
   for(i=0;i<height;i++)//creating blown map
   {
      for(j=0;j<width;j++)
      {
         ...
         if(grid_map.data[i*width+j]==70)//blowing lane data
         {
               ...
               for(l=-1*minor_axis_in_cells;l<=minor_axis_in_cells;l++)
               {
                  if(((minor_axis_in_cells*minor_axis_in_cells)*(k*k) + (major_axis_in_cells*major_axis_in_cells)*(l*l)) < (minor_axis_in_cells*minor_axis_in_cells)*(major_axis_in_cells*major_axis_in_cells))//inside ellipse condition
                  {
                     ...
                     if(x >= 0 && x <= range)
                     {
                        if(y <= range && y >= -range)
                        {
                           blown_grid_map.data[x*width+y+width/2] = 70;
                        }
                     }
                  }
               }
   ...
   Mat path_map = Mat::zeros(height,width,CV_8UC1);
   for(i=0;i<height;i++)
   {
     for(j=0;j<width;j++)
     {
       ...
       path_map.at<uchar>(i,j) = grid_map.data[i_*width+j]*2.55;
     }
   }
   if(path_recieved)
   {
      if(!path.poses.empty())
      {
         ...
         path_map.at<uchar>( height - (int)(path.poses[i].pose.position.y/map_res)-1,(int)(path.poses[i].pose.position.x/map_res) ) = 255;
...
void path_disp(const nav_msgs::Path::ConstPtr& msg)
{
   if(!msg->poses.empty())
   path = (*msg);
   path_recieved = true;
...
{
   ros::init(argc,argv,"local_map_node_rise");
   ros::NodeHandle n,n1,n2,n3;
   ros::Subscriber lane_sub=n.subscribe<nav_msgs::OccupancyGrid>("/Lane_Occupancy_Grid",10,laneCallback);
   ros::Subscriber lidar_sub=n1.subscribe<sensor_msgs::LaserScan>("scan",10,lidarCallback);
   map_pub=n2.advertise<nav_msgs::OccupancyGrid>("scan/local_map",10);
   blown_map_pub=n3.advertise<nav_msgs::OccupancyGrid>("scan/blown_local_map",10);
   ros::Subscriber path=n.subscribe<nav_msgs::Path>("/path",10,path_disp);

   namedWindow("local_map",CV_WINDOW_NORMAL);
   namedWindow("path_map",CV_WINDOW_NORMAL);
