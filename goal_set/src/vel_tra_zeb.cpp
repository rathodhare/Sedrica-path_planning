...
double decc=2.5;
double cacc = 1.7;
double v_max = 5.55,v_zebra = 0.5;
double weight = 0.7;
bool output_vel;

int velD_curr_can=0;
float velD_curr=velD_curr_can*5/18;
ros::Publisher vel,dir;
std_msgs::Float64MultiArray v,v_copy,directions,directions_copy;
...
void traffic_dir_init(const std_msgs::Int64::ConstPtr& tra){
	//cout<<"called!!";
	tr_dir = (tra->data);
}

void zebra_init(const std_msgs::Float64::ConstPtr& zeb){
	//cout<<"zebra_init";
	//cout<<" zeb data "<<zeb->data;
		ze = zeb->data;
}
...
if(!msg->poses.empty()){
 ...
 // double x[no_data], y[no_data];
  double r[no_data-2];
 	...
 	directions.data.push_back(msg->poses[no_data-1].pose.position.z);
 	directions.data.push_back(msg->poses[no_data-2].pose.position.z);
 	///v[no_data-3]=v_out;
	vector<double> s;
  vector<double> v_m;
  vector<vector<double> > segment_v_m;
	...
	double z1 = msg->poses[i-1].pose.position.z ,z2 = msg->poses[i].pose.position.z, z3 = msg->poses[i+1].pose.position.z;
	directions.data.push_back(z2);
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber output_velocity=n.subscribe("/output_vel",1,output_vel_init);
	...
	ros::Subscriber path=n.subscribe("/path",1,path_vel); 
 	...
 	dir = n.advertise<std_msgs::Float64MultiArray>("/car_desired_dir",1);
