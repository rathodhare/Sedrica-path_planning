...
double des_lat[co_number] = {19.129447905};//{19.1297056083};//{19.1294746417};//{ 19.1251380983};//{19.1259112833};////{19.1296389633};//{19.1294204367};//{19.1295804183};    //Define Required GPS location //gate 19.1295974467
double des_lon[co_number] = {72.9171624267};//{72.9182976383};//{72.9171582767};//{72.9159026833};//{72.9155033633};//{72.9155408533};////{72.9172522317};//{72.91613313}; // gate 72.9173579667
double des_xpos=0;
double des_ypos=0;
...
float traffic_frames_count = 0,traffic_not_count = 0,traffic_confidence = 0.8,traffic_imu_angle_ini = 0;
int x_near,y_near,x_far,y_far;
int traffic_frames_limit = 30;

string zone;
string data;

bool final_goal_reached=false,car_start = false,traffic_move = true,traffic_action_done = false,traffic_action_first = true;
...
void traffic_dir(std_msgs::Int8 msg)
{
	...
	{
		traffic_frames_count++;
	}
	if(traffic_frames_count>0)
	{		
		traffic_action_done = false;
		if(msg.data != 1 && msg.data != 2)
		{
			traffic_not_count++;
		}
...
int main(int argc,char** argv)
{
   ...
   ros::Subscriber coord_sub = n.subscribe("/lane_coord", 10, lane_coord);
   ...
   while(ros::ok())
   {
        ...
        if(sin((90-map_theta)*DEG_TO_RAD)>=0&&cos((90-map_theta)*DEG_TO_RAD)>=0) //1st quadrant
  		...
  		// traffic blocking
  		if(traffic_direc.data == 1 || traffic_direc.data == 2||(traffic_frames_count/(traffic_frames_count +traffic_not_count)>=traffic_confidence && !traffic_action_done))
		  	{
		  		if(traffic_action_first)
		  		{
		  			traffic_imu_angle_ini = curr_head;
		  			traffic_action_first = false;
		  		}
		  		if(abs(curr_head- traffic_imu_angle_ini) >=60 )
		  		{
		  			traffic_action_done = true;
		  			traffic_frames_count =0;
		  			traffic_not_count =0;
		  		}
				if(traffic_direc.data == 1)
			  	{
			  		x_near = 0; x_far = width - 1;
