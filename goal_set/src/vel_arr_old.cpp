...
double decc=3.5;
double cacc = 1.7;
double v_max = 5.55,v_zebra = 2.77;
double weight = 0.7;
bool output_vel;
...
void traffic_init(const std_msgs::Float64MultiArray::ConstPtr& tra){
	tr.clear();
	//cout<<"called!!";
	for(int i=0;i<tra->data.size();i++)
	{
	//	cout<<"yo_tr";
		tr.push_back(tra->data[i]);
	}
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber output_velocity=n.subscribe("/output_vel",1,output_vel_init);
	...
	ros::Subscriber traffic=n.subscribe("/traffic",1,traffic_init);
	ros::Subscriber zebra=n.subscribe("/zebra",1,zebra_init);
