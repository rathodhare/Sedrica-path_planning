...
//for 2d astar output
void Smoother::tracePath_a(const Node2D* node, int i, std::vector<Node3D> path) {
  if (node == nullptr) {
    this->path = path;
    return;
  }
  ...
  i++;
  Node3D node3d_a(0, 0, 0, 0, 0, nullptr);
  float x = (float)node->getX();
  float y = (float)node->getY();
  node3d_a.setX(x);
  node3d_a.setY(y);
  path.push_back(node3d_a);
  tracePath_a(node->getPred(), i, path);
}
