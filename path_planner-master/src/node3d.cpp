...
//custom
//const float Node3D::dy[] = { 0.0,        -0.032869,  0.032869};
//const float Node3D::dx[] = { 0.67932,   0.67917,   0.67917};
//const float Node3D::dt[] = { 0.0,         0.080472,   -0.080472};
/*
//sliding custom
namedWindow("Node3D_params",CV_WINDOW_AUTOSIZE);
...
bool Node3D::isOnGrid(const int width, const int height) const {
  bool c = ((t / Constants::deltaHeadingRad) >= 0);
  bool d = ((t / Constants::deltaHeadingRad) < Constants::headings);
  //std::cout<<(x >= 0)<<" "<<(x < width)<<" "<<(y >= 0)<<" "<<(y < height)<<" "<<c<<" "<<d<<std::endl; 
